...
#define UPPER_TEMPERATURE  20.0
#define LOWER_TEMPERATURE  15.0
...
extern struct _led_state led_state[LEDS_QUANTITY];

#define TAG "TemperatureReader"

#define CID_ESP 0x02E5

bool isBound = false;
bool isInitialStatusValuesReceived = false;
...
void ble_mesh_send_gen_onoff_set()
{
    ...
    set.onoff_set.onoff = led_state[0].current;
    ...
    store.onOff = set.onoff_set.onoff;
    
    printf("store.onOff = %d \n", store.onOff);

    mesh_info_store(); /* Store proper mesh example info */
}

void getCurrentServerStatus(){
    esp_err_t err = ESP_OK;
    esp_ble_mesh_client_common_param_t common = {0};
...
void updateActuatorStatus(uint8_t OnOff){
    if(led_state[0].current != OnOff){
        led_state[0].current = OnOff;
    } else {
        ESP_LOGI(TAG,"led_state[0].current already updated!");
    }
}

static void *PIR_task(void * arg)
{
    ESP_LOGI(TAG, "%s: Starting PIR read Task\n\n", __func__);

    while(!isBound){
        ESP_LOGI(TAG,"%s: Missing APP Key Bind", __func__);
        vTaskDelay(3000 / portTICK_RATE_MS);
    }

    getCurrentServerStatus();
    while(!isInitialStatusValuesReceived){
        ESP_LOGI(TAG,"%s: Waiting for the reception of Server status", __func__);
        vTaskDelay(3000 / portTICK_RATE_MS);
    }
    ESP_LOGI(TAG,"%s: Received the initial values from server nodes", __func__);

    while (1)
    {
        ESP_LOGI(TAG, "=== Reading PIR ===\n");

        led_state[0].previous = led_state[0].current;
        
        readPIR();

        if(led_state[0].current != led_state[0].previous){
            ble_mesh_send_gen_onoff_set();
        } else {
            ESP_LOGI(TAG, "led_state[0].current is equal to led_state[0].old");
        }
                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_GET_STATE_EVT:
        ...
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET) {
            ...
            updateActuatorStatus(param->status_cb.onoff_status.present_onoff);
            
            if(!isInitialStatusValuesReceived){
                isInitialStatusValuesReceived = true;
            }
        }
        break;
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT, onoff %d", 
                    param->status_cb.onoff_status.present_onoff);
        if(!isInitialStatusValuesReceived){
            updateActuatorStatus(param->status_cb.onoff_status.present_onoff);
            isInitialStatusValuesReceived = true;
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_TIMEOUT_EVT:
        ...
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
            ...
            ESP_LOGI(TAG, "Timeout event happened with ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET message");
        }
        break;
    default:
        break;
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
            ...
            if (param->value.state_change.mod_app_bind.company_id == 0xFFFF &&
                ...
                ESP_LOGI(TAG,"%s: Inside if (param->value.state_change.mod_app_bind.model_id == ESP_BLE_MESH_VND_MODEL_ID_TEMPERATURE_CLIENT",
                              __func__);
...
void app_main(void)
{
    ...
    (void)pthread_create(&threadPIRRead, NULL, PIR_task, NULL);
}
