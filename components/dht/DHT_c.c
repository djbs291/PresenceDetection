...
float humidity = 0.;
float temperature = 0.;
uint16_t unsignedTemperature = 0x0000;

struct _dht_state dht_state[1] = {
    {INITIAL_TEMPERATURE, INITIAL_TEMPERATURE, false, HEATER_OFF, HEATER_OFF, FAN_OFF,FAN_OFF},
};
...
uint16_t getUnsignedTemperature() { return unsignedTemperature; }
...
#define MAXdhtData 5 // Bytes to complete 40 = 5*8 Bits
...
int readDHT()
{
    ...
    // Set the pin direction as output in order to be able to send data requests do DHT
    ...
    gpio_set_direction(DHTgpio, GPIO_MODE_INPUT); // change to input mode to receive the requested data
    ...
    humidity = dhtData[0];
    humidity *= 0x100; // >> 8 / 0x100 ??
    humidity += dhtData[1];
    humidity /= 10; // get the decimal

    // == get temp from Data[2] and Data[3]

    temperature = dhtData[2] & 0x7F; // 0x7F ??
    temperature *= 0x100; // >> 8 / 0x100 ??
    temperature += dhtData[3];
    unsignedTemperature = temperature;

    temperature /= 10;


    if (dhtData[2] & 0x80) // negative temp, brrr it's freezing / 0x80 ?
        temperature *= -1;
